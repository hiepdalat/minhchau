// === server.js: Gộp đầy đủ congno + nhaphang + banhang, giữ session 5 phút idle mới logout ===
...
app.post('/api/stock/receive', requireLogin, async (req, res) => {
  try {
    ...
    const mapped = items.map(it => ({
      ...
      dongia: it.price,
      ck: it.discount,
      gianhap: it.price * (1 - it.discount / 100),
      thanhtien: it.qty * it.price * (1 - it.discount / 100)
    }));
